...
public class BatteryMeterViewBamz extends View implements DemoMode,
    ...
    private class BatteryTracker extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(Intent.ACTION_BATTERY_CHANGED)) {
                ...
                setContentDescription(context.getString(ComotID.Get("accessibility_battery_level","string"), level));
    ...
    public BatteryMeterViewBamz(Context context) {
        this(context, null, 0);
    }

    public BatteryMeterViewBamz(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public BatteryMeterViewBamz(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray atts = context.obtainStyledAttributes(attrs, ComotID.GetStyle("BatteryMeterView"),
                defStyle, 0);
        mFrameColor = atts.getColor(ComotID.Get("BatteryMeterView_frameColor","styleable"),
                res.getColor(ComotID.Get("batterymeter_frame_color","color")));
        TypedArray levels = res.obtainTypedArray(ComotID.Get("batterymeter_color_levels","array"));
        TypedArray colors = res.obtainTypedArray(ComotID.Get("batterymeter_color_values","array"));
        ...
        mWarningString = context.getString(ComotID.Get("battery_meter_very_low_overlay_symbol","string"));
        mCriticalLevel = getContext().getResources().getInteger(
                ComotID.Get("config_criticalBatteryWarningLevel","integer"));
        mButtonHeightFraction = context.getResources().getFraction(
                ComotID.Get("battery_button_height_fraction","fraction"), 1, 1);
        mSubpixelSmoothingLeft = context.getResources().getFraction(
                ComotID.Get("battery_subpixel_smoothing_left","fraction"), 1, 1);
        mSubpixelSmoothingRight = context.getResources().getFraction(
                ComotID.Get("battery_subpixel_smoothing_right","fraction"), 1, 1);

        mDarkModeBackgroundColor =
                context.getResources().getColor(ComotID.Get("dark_mode_icon_color_dual_tone_background", "color"));
        mDarkModeFillColor = context.getResources().getColor(ComotID.Get("dark_mode_icon_color_dual_tone_fill", "color"));
        mLightModeBackgroundColor =
                context.getResources().getColor(ComotID.Get("light_mode_icon_color_dual_tone_background", "color"));
        mLightModeFillColor = context.getResources().getColor(ComotID.Get("light_mode_icon_color_dual_tone_fill", "color"));
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int width = (int) (19.0 / 10.0 * anuSize);
        int height = (int) (25.0 / 10.0 * anuSize);
        ...
        if (mMeterMode == BatteryMeterMode.BATTERY_METER_CIRCLE) {
            ...
            width = height;
            //Toast.makeText(getContext(), "C Width:" + width + " C Height:" + height, Toast.LENGTH_SHORT).show();
            ...
            //Toast.makeText(getContext(), "I Width:" + width + "I Height:" + height, Toast.LENGTH_SHORT).show();
        }
        ...
        if(mBatteryStyle != 1 && mBatteryAlign == 2) {
            ...
            switch (style) {
                ...
                case BatteryController.STYLE_GONE:
                    ...
                    break;
                // MOD
                case BatteryController.STYLE_ICON_PORTRAIT:
            ...
            setMode(BatteryMeterMode.BATTERY_METER_GONE);
            mShowPercent = false;
            invalidate();
        }
       /* boolean showInsidePercent = percentMode == BatteryController.PERCENTAGE_MODE_INSIDE;
    ...
    private int getColorForDarkIntensity(float darkIntensity, int lightColor, int darkColor) {
        ...
        return (int) new ArgbEvaluator().evaluate(darkIntensity, lightColor, darkColor);
    }
    ...
    protected class NormalBatteryMeterDrawable implements BatteryMeterDrawable {
        ...
        public NormalBatteryMeterDrawable(Resources res, boolean horizontal) {
            ...
            mChargeColor = getResources().getColor(ComotID.Get("batterymeter_charge_color", "color"));

            mBoltPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
            //mBoltPaint.setColor(res.getColor(R.color.batterymeter_bolt_color));
            mBoltPaint.setColor(res.getColor(ComotID.Get("batterymeter_bolt_color", "color")));
        ...
        public void onDraw(Canvas c, BatteryTracker tracker) {
            ...
            // MOD
            mIconTint = mBatteryColor;
            mBoltPaint.setColor(mBatteryColor);
            mChargeColor = mBatteryColor;
            // End
        ...
        public void onSizeChanged(int w, int h, int oldw, int oldh) {
            ...
            mWidth = w;
            // MOD
            //Toast.makeText(getContext(), "Old Width:" + oldw + "Old Height:" + oldh, Toast.LENGTH_SHORT).show();
            //Toast.makeText(getContext(), "Width:" + mWidth + "Height:" + mHeight, Toast.LENGTH_SHORT).show();
            //mHeight = 26;
            //mWidth = 26;
            // End
            // 44x37 land 189x37 port
        ...
        private float[] loadBoltPoints(Resources res) {
            ...
            final int[] pts = res.getIntArray((mHorizontal
                    ? ComotID.Get("batterymeter_inverted_bolt_points", "array")
                    : ComotID.Get("batterymeter_bolt_points", "array")));
            int maxX = 0, maxY = 0;
    ...
    protected class CircleBatteryMeterDrawable implements BatteryMeterDrawable {
        ...
        public CircleBatteryMeterDrawable(Resources res) {
            ...
            mBackPaint.setColor(res.getColor(ComotID.Get("batterymeter_frame_color", "color")));
        ...
        private float[] loadBoltPoints(Resources res) {
            ...
            final int[] pts = res.getIntArray(ComotID.Get("batterymeter_bolt_points", "array"));
            int maxX = 0, maxY = 0;
            for (int i = 0; i < pts.length; i += 2) {
                maxX = Math.max(maxX, pts[i]);
                maxY = Math.max(maxY, pts[i + 1]);
        ...
        private void initSizeBasedStuff() {
            ...
            //MOD
            //mCircleSize = Math.min(getMeasuredWidth()/100*75, getMeasuredHeight()/100*75);
            //mCircleSize = Math.min(getMeasuredWidth()/100*75, getMeasuredHeight()/100*75);
            //Toast.makeText(getContext(), "Circle Size: " + mCircleSize, Toast.LENGTH_SHORT).show();
            //mCircleSize = Math.min(28, 28);
            //Toast.makeText(getContext(), "Circle Size: " + mCircleSize, Toast.LENGTH_SHORT).show();
            //Toast.makeText(getContext(), "Width:" + getMeasuredWidth() + "Height:" + getMeasuredHeight(), Toast.LENGTH_SHORT).show();
            // End
